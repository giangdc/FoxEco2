...
> **Cập nhật 2026-07-31 (bổ sung #25 — C-ORD-06 Resolved với ảnh chụp thật + notification click-navigate):** (1) User bổ sung 4 ảnh chụp THẬT app STG vào `00_input/v1.0/Ảnh danh sách rỗng app stag thật/` xác nhận text empty-state chính thức cho 3 màn (khác nhau, không dùng chung 1 text generic như trước) — Hoạt động = "Chưa có đơn nào", Thông báo = "Chưa có thông báo nào", Quà đã nhận = "Chưa có quà nào" + subtitle "Quà bạn nhận được từ đồng nghiệp sẽ hiện ở đây". `C-ORD-06` chuyển lại **Resolved** (lần bị revert 2026-07-29 là do thiếu bằng chứng cụ thể — lần này có ảnh kèm theo). Sửa Expected Result `SC-ORD-023`/`SC-GIFT-006`/`SC-NTF-007` theo đúng text mới. **(2)** User xác nhận qua chat 9 hành vi click-notification → điều hướng màn đích (module NTF) — mở rộng scope cho `SC-NTF-001..005` (tap → Theo dõi đơn/Chi tiết tin/Quà đã nhận tuỳ loại), viết tách 9 TC riêng (không gộp). Cố ý **không** thêm case "Tin quá hạn" (NTF-09) và "Nhắc khung giờ hẹn giao" theo yêu cầu user. `C-NTF-01` (danh sách chính thức 9 loại thông báo) **vẫn giữ Open** — user xác nhận đây chỉ là baseline làm việc tạm, chưa phải câu trả lời chốt của BA. **KHÔNG đụng module Trang chủ** (block "Đơn của tôi"/"Tin mới") — còn đang chờ user trả lời riêng câu hỏi khác (hành vi ẩn/hiện section + nguồn text "Tin mới" rỗng chưa có bằng chứng). Chi tiết: `test_scenario_map.md` Block "Click thông báo → điều hướng" (NTF) + Block "Empty state" (Hoạt động/Quà đã nhận/Thông báo). **Đã generate-tc + consolidate xong (2026-07-31):** sửa 3 TC text (`TC_01.19/20`, `TC_02.11`, `TC_03.19`) + thêm 9 TC mới (`TC_03.31..39`) vào fragment rồi consolidate lại `ISC_FoxEco_v1.0_TC_v1_R1.xlsx` + 2 alias. TC-MASTER 337→**346**. Xem `MEMORY.md §9` dòng cuối + `03_test-cases/v1.0/CHANGELOG.md`. **Bước tiếp theo:** `/review-tc --recheck` cho bộ 346 TC; sau đó `/analyze-requirements --update` cho module Trang chủ khi user trả lời xong 2 câu hỏi còn treo (block "Đơn của tôi" ẩn/hiện khi rỗng, nguồn text "Tin mới" rỗng).
> **Cập nhật 2026-07-31 (bổ sung #26 — module Trang chủ, ảnh chụp thật + CORRECTED rule sai):** User bổ sung ảnh chụp thật màn Trang chủ (`00_input/v1.0/Ảnh danh sách rỗng app stag thật/Trang chủ.jpg`), trả lời nốt 2 câu hỏi còn treo ở bổ sung #25. **Phát hiện quan trọng:** rule gốc trích từ `DOC-v1.0-02` cho block "Đơn của tôi" — *"Chỉ hiện khi có đơn đang hoạt động"* — **SAI so với UI thật**: section này LUÔN hiển thị (có header + link "Xem tất cả"), khi rỗng hiện text "Chưa có đơn nào" thay vì ẩn cả section. TC cũ (`TC_05.15` vùng lân cận) đang test theo rule sai (assert "section KHÔNG xuất hiện") — cần viết lại. +1 scenario **`SC-ORD-037`** (CORRECTED). Đồng thời phát hiện block "Tin mới" **chưa từng có scenario rỗng** — bổ sung text thật từ ảnh: title "Chưa có tin mới" + subtitle "Hiện chưa có tin nào gần bạn, quay lại sau nhé" → +1 scenario mới **`SC-ASN-015`**. Cả 2 đã thêm vào `test_scenario_map.md` (Block "Đơn của tôi"/"Tin mới", Trang chủ) + `requirement_traceability.md` (`REQ-ORD-001`, `REQ-ASN-005`) + §4 Scenario Index. Tổng scenario 98→**100**. **Bước tiếp theo:** `/generate-tc` sửa TC sai + thêm TC mới cho `fragments/TC-TRANGCHU-v1.0.xlsx`, rồi consolidate lại TC-MASTER.
> **Cập nhật 2026-08-03 (bổ sung #27 — vibe-test VR-002, module Hoạt động):** `/vibe-test` chạy full sheet Hoạt động (`TC_01.1→21`, 21/21 TC) qua Appium MCP trên app FoxPro STG thật. Kết quả ban đầu **7 PASS / 5 FAIL / 9 BLOCKED** — chi tiết run gốc ở `08_test-runs/vibe/VR-002-2026-08-03/vibe-report.md`. 3 finding phát sinh, đã **RESOLVED ngay trong cùng phiên (2026-08-04) — user xác nhận trực tiếp qua chat: cả 3 đều là gap tài liệu/TC, KHÔNG phải bug app thật:**
> 1. **"Đã giao" tại tab "Đang diễn ra"** — xác nhận là trạng thái trung gian hợp lệ (giữa "Đang giao" và "Hoàn thành"). Đã sửa `SC-ORD-017` + `TC_01.4` (Fail→Pass).
> 2. **Card thiếu trường "ngày"** — xác nhận: trường ngày (ngày hoàn thành) CHỈ có ở card tab "Đã hoàn thành", tab "Đang diễn ra" vốn dĩ không có. Đã sửa `SC-ORD-018` (bỏ ngày) + `SC-ORD-019` (thêm ngày hoàn thành, còn cần vibe-test xác nhận trực tiếp khi có data vì tab luôn rỗng) + `TC_01.8` (Fail→Pass) + `TC_01.11` (Expected bổ sung, vẫn BLOCKED chờ data).
> 3. **Tap card → "Theo dõi đơn" thay vì "Chi tiết tin"** — xác nhận đây là hành vi ĐÚNG, TC-MASTER ghi sai đích điều hướng. Đã sửa `SC-ORD-021` + `SC-ORD-024` + `TC_01.13/14/15` (Fail→Pass) + `TC_01.16/17` (sửa wording, vẫn BLOCKED chờ data).
>
> **Kết quả cuối cùng sau fix (2026-08-04):** Hoạt động = **12 PASS / 0 FAIL / 9 BLOCKED** (21 TC). Đã đồng bộ `test_scenario_map.md` (4 scenario CORRECTED, xem block "SC-ORD-015..026" Analyst Note) + `TC-MASTER-LATEST.xlsx` (Round 1, recalc LibreOffice sạch). Không mở clarification mới — user tự xác nhận trực tiếp, không cần chờ BA riêng.
> **Cập nhật 2026-08-04 (bổ sung #28 — vibe-test VR-003, module Thông báo):** `/vibe-test` chạy full sheet Thông báo (`TC_03.1→39`, 39/39 TC) qua Appium MCP, flow Trang chủ → tap icon chuông → màn Thông báo (đúng yêu cầu user), account test "Phan Minh Tài". Kết quả: **10 PASS / 0 FAIL / 29 BLOCKED** — chi tiết `08_test-runs/vibe/Thongbao_VR-006-2026-08-05/vibe-report.md` (⚠ 2026-08-05: run gốc `VR-003-2026-08-04` đã được merge vào folder `Thongbao_VR-006-2026-08-05` — xem update #30 bên dưới — folder `VR-003-2026-08-04` KHÔNG còn tồn tại). **KHÔNG phải bug app** — BLOCKED do 3 nhóm nguyên nhân ngoài phạm vi 1 tài khoản/1 thiết bị: (1) 16 TC (`TC_03.1-16`) cần orchestrate đa vai trò Sender/Receiver/Carrier trong cùng đơn (đề xuất chạy `/vibe-test-multi-device` riêng); (2) 8 TC (`TC_03.29-32/34/37-39`) cần loại thông báo không tồn tại trong lịch sử tài khoản test hiện tại (chỉ có 3/9 loại: "Đơn đã được giao", "Đơn gửi tới bạn đã có người vận chuyển", "Người vận chuyển đã lấy hàng"); (3) `TC_03.19` cần tài khoản rỗng, `TC_03.25-28` cần dữ liệu đủ lớn để verify phân trang (list hiện tại load hết 1 lần, không quan sát được cơ chế load-more, và **không có chỉ báo "hết dữ liệu"** ở cuối danh sách — ứng viên gap UI cho `TC_03.26`, cần BA/dev xác nhận). **10 PASS đáng chú ý:** cấu trúc màn hình đúng đủ (`TC_03.17`), icon back hoạt động đúng (`TC_03.18`), tap 1 thông báo → chỉ đánh dấu đã đọc đúng thông báo đó không ảnh hưởng thông báo khác (`TC_03.20`), nút "Đánh dấu đã đọc" mark-all hoạt động đúng và **tự ẩn khi hết unread** thay vì hiện dạng no-op (`TC_03.21`/`TC_03.23`, hành vi hợp lý), tap lại thông báo đã đọc không lỗi (`TC_03.22`), 3/9 loại thông báo test click-navigate đều đúng đích "Theo dõi đơn" (`TC_03.33/35/36`). **Đã fix luôn 15 dòng lỗi công thức off-by-one (cột AM/AO)** trong sheet Thông báo — bug có sẵn từ trước (ghi nhận ở VR-002 report nhưng chưa fix), ảnh hưởng trực tiếp Status của TC_03.18–TC_03.30 nên fix dứt điểm trong run này; 7 sheet khác (~207 dòng) vẫn CHƯA fix, ngoài phạm vi. Đã ghi Round 1 vào TC-MASTER Excel + recalc LibreOffice sạch (39/39 khớp AM/AO/Dashboard). Không mở clarification mới.
> **Cập nhật 2026-08-04 (bổ sung #29 — vibe-test VR-004, module Cá nhân, 2 account):** `/vibe-test` chạy full sheet Cá nhân (`TC_02.1→17`, 17/17 TC) qua Appium MCP, **2 round trên 2 account khác nhau** theo yêu cầu user: Round 1 = stag_TaiPM@fpt.com (data 0 đơn đã giúp/0 quà đã nhận), Round 2 = stag_giangdc2@fpt.com (data 1 đơn đã giúp/1 quà đã nhận — trùng khớp tự nhiên với boundary=1 của `TC_02.6`). Kết quả: **R1 6 PASS/4 FAIL/7 BLOCKED · R2 9 PASS/3 FAIL/5 BLOCKED** — chi tiết `08_test-runs/vibe/Profile_VR-007-2026-08-05/vibe-report.md` (⚠ 2026-08-05: folder gốc `VR-004-2026-08-04` đã merge vào `Profile_VR-007-2026-08-05` cùng Round 3 rồi xoá theo yêu cầu user). **3 finding đáng chú ý (chưa Resolved, cần BA/dev xác nhận — khác VR-002/003, lần này KHÔNG tự ý sửa TC vì thiếu căn cứ đủ mạnh):**
> 1. **`TC_02.1`/`TC_02.2` FAIL cả 2 round** — ban đầu ghi nhận UI thật chỉ hiển thị 3/6 trường (tên, avatar, phòng ban), thiếu SĐT/khu vực/kênh liên hệ. **User xác nhận trực tiếp qua chat (2026-08-04) ngay sau khi nhận report:** SĐT/khu vực/kênh liên hệ **KHÔNG phải gap thật** — 6-field list gốc trích từ BRD text (USR-02) đã lỗi thời, KHÔNG cần thiết ở UI v1.0. **Gap THẬT DUY NHẤT ở 2 TC này là thiếu trường MNV** (mã nhân viên) — đã sửa lại Expected Result `TC_02.1` (6→4 trường: tên, avatar, phòng ban, MNV) và `TC_02.2` (đổi từ "phòng ban + khu vực/tỉnh" sang "phòng ban + MNV", khớp đúng ví dụ mẫu "Phòng Kỹ thuật · MNV: FTEL2291" vốn đã có sẵn trong Expected cũ — tự sửa luôn 1 mâu thuẫn nội bộ TC). Cả 2 TC vẫn giữ nguyên kết quả **FAIL** (lý do hẹp lại còn thiếu MNV). Đã sửa fragment `TC-CANHAN-v1.0.xlsx` + `TC-MASTER-LATEST.xlsx` trực tiếp (không qua generate-tc, do user xác nhận nhanh qua chat).
> 2. **`TC_02.3` FAIL cả 2 round (phần badge tier)** — Expected liệt kê "(4) badge tier 🏆 Hạng Đồng hành" là 1 trong 6 phần tử bắt buộc của header, UI thật KHÔNG có badge này. **User xác nhận (2026-08-04): đây LÀ gap thật, KHÔNG phải TC drift** — khớp đúng tinh thần `C-USR-01` (Resolved — chỉ cấm viết TC test LOGIC phân hạng/tính tier, không cấm hiển thị badge display-only tĩnh: "nếu badge tier xuất hiện, chỉ hiển thị dạng text tĩnh, không có cơ chế tính toán" — nghĩa là badge NÊN xuất hiện dạng tĩnh). TC_02.3 giữ nguyên KHÔNG sửa (Expected đã đúng từ đầu). **Cả 2 gap (MNV + badge tier) đã ghi nhận `C-USR-04` + `/log-bug` ngay trong phiên (2026-08-04): `BUG-003` (Medium/P3, gộp cả MNV + badge tier theo yêu cầu user — cùng 1 màn/1 lần phát hiện) — xem `05_bug-reports/bug-index.md`.**
> 3. **`TC_02.11` FAIL ở Round 1 (account 0 quà)** — Expected empty-state title = "Chưa có quà nào" (xác nhận từ ảnh chụp thật STG, `C-ORD-06` Resolved 2026-07-31), nhưng build STG lúc vibe-test (2026-08-04) lại hiển thị text generic **"Hiện tại chưa có dữ liệu"**. Subtitle vẫn đúng ("Quà bạn nhận được từ đồng nghiệp sẽ hiện ở đây"). Đây là **regression khả nghi** (build cũ hơn thời điểm chụp ảnh, hoặc bị revert) — **đề xuất `/log-bug` thay vì sửa TC**, vì đã có bằng chứng ảnh xác nhận rõ ràng cho text đúng.
> **9 PASS đáng chú ý:** 2 chỉ số đóng góp hiển thị đúng cả case 0 và case boundary=1 (`TC_02.4/5/6`), điều hướng "Đơn của tôi"/"Quà đã nhận" đúng đích (`TC_02.8/9`), xác nhận KHÔNG có UI cấu hình kênh liên hệ (`TC_02.10`, khớp `C-USR-02` Resolved/Deferred), icon quay lại hoạt động đúng (`TC_02.15`), card "Quà đã nhận" chỉ load đúng loại đã nhận + danh sách lịch sử khớp dữ liệu thật (`TC_02.12`), đủ 3 thành phần trạng thái có dữ liệu (`TC_02.16`). **12 TC BLOCKED do thiếu dữ liệu dàn dựng** (boundary ngược giữa 2 round, số liệu lớn ≥999/≥12, đủ 4/4 loại quà, ≥2 record lịch sử từ ≥2 người tặng) — không dàn dựng được qua UI thông thường, cần QA lead seed data riêng nếu muốn phủ hết. Đã ghi cả 2 round vào TC-MASTER Excel (Round 1 = cột N-R, Round 2 = cột S-W) + recalc LibreOffice sạch. Không mở clarification mới trong phiên này — 3 finding trên để BA/dev xác nhận trước khi quyết định hướng xử lý (sửa TC hay log bug).
> **Cập nhật 2026-08-05 (bổ sung #30 — vibe-test VR-006, module Thông báo Round 2 + merge folder):** `/vibe-test` retest 29 TC pending còn lại của module Thông báo (`TC_03`, sau Round 1/VR-003 2026-08-04: 10 PASS/29 BLOCKED), account "Phan Minh Tài" (session có sẵn), ghi vào Round 2. Kết quả Round 2: **5 PASS / 0 FAIL / 24 BLOCKED**. **5 TC chuyển BLOCKED→PASS nhờ dữ liệu tài khoản tự nhiên phát sinh thêm** (KHÔNG phải seed thủ công hay sửa app) — tài khoản dùng chung nhiều tester nên có thêm hoạt động trong ngày: `TC_03.37` (loại thông báo mới "Đơn đã hoàn tất" xuất hiện lần đầu, tap → đúng "Theo dõi đơn"), `TC_03.39` (loại mới "Đơn đã bị huỷ" xuất hiện lần đầu, tap → đúng "Theo dõi đơn" với banner huỷ), `TC_03.25/26/27` (list đủ sâu hơn — 3 nhóm HÔM NAY→HÔM QUA→TUẦN NÀY — đã quan sát được cơ chế load-more hoạt động đúng, không dup, không crash khi scroll quá đáy; TC_03.26 PASS theo đúng câu chữ Expected dù UI không có text "hết dữ liệu" — ghi nhận lại làm UX suggestion, không phải bug). **24 TC còn lại vẫn BLOCKED, cùng nguyên nhân Round 1** (không đổi): 16 TC multi-role (`TC_03.1-16`), `TC_03.19` cần tài khoản rỗng, `TC_03.28` (mất mạng giữa lúc load — thử qua `adb shell svc wifi/data disable` nhưng list đã hết dữ liệu trước khi kịp cắt mạng nên không canh đúng timing), `TC_03.29/30` (SĐT đã lộ — cần trigger real-time), `TC_03.31/32/34/38` (4/9 loại thông báo vẫn chưa từng phát sinh trên tài khoản test — tổng đã xác nhận 5/9 loại qua 2 round). **Tổng tích luỹ module: 15 PASS / 0 FAIL / 24 BLOCKED / 39 TC.**
> **Đồng thời đã merge folder run gốc `VR-003-2026-08-04` (Round 1) vào folder `Thongbao_VR-006-2026-08-05` (Round 2) theo yêu cầu user** — gộp đủ 4 file (vibe-report/vibe-log/vibe-locators/mcp-session-log, giữ 2 section Round 1 + Round 2 riêng biệt trong mỗi file) + 13 screenshot Round 1 (tổng 17 screenshot), verify đủ 39/39 TC ID + đủ screenshot trước khi xoá, sau đó **đã xoá folder `VR-003-2026-08-04` gốc**. Đã sửa các path reference trỏ tới folder cũ trong `vibe-locators-latest.md` + entry #28 phía trên. Chi tiết đầy đủ `08_test-runs/vibe/Thongbao_VR-006-2026-08-05/vibe-report.md`. Không mở clarification mới. Excel Round 2 (cột S-W sheet Thông báo) ghi sau khi file `TC-MASTER-LATEST.xlsx` được đóng trong LibreOffice (đang mở tại thời điểm test).
...
| ORD | DOC-v1.0-01, DOC-v1.0-02, quan sát thực tế app | 15 | 36 | 36 | 0 | 0 | 0 | 5 | 18 | 13 | Medium |
| ASN | DOC-v1.0-01, DOC-v1.0-02, DOC-v1.0-03 | 10 | 14 | 14 | 0 | 0 | 0 | 7 | 5 | 2 | High |
| DLV | DOC-v1.0-01, DOC-v1.0-02, DOC-v1.0-03, quan sát thực tế app | 6 | 15 | 15 | 0 | 0 | 0 | 5 | 4 | 6 | Medium |
| GIFT | DOC-v1.0-01, DOC-v1.0-02, quan sát thực tế app | 3 | 8 | 8 | 0 | 0 | 0 | 0 | 2 | 6 | Low |
| CNL | DOC-v1.0-01, DOC-v1.0-02 | 3 | 5 | 5 | 0 | 0 | 0 | 2 | 3 | 0 | Low |
| NTF | DOC-v1.0-01, DOC-v1.0-02, DOC-v1.0-04, quan sát thực tế app | 2 | 11 | 10 | 0 | 0 | 1 | 1 | 4 | 5 | Medium |
| TS | DOC-v1.0-01 | 2 | 3 | 3 | 0 | 0 | 0 | 0 | 2 | 1 | Low |
| **Tổng** | | **46** | **99** | **98** | **0** | **0** | **1** | **21** | **42** | **35** | |
...
| SC-USR-002 | Xem hồ sơ cá nhân (view-only — xác nhận không có update, C-USR-03) | USR | DOC-v1.0-01 | P2 | Functional | NEW | ✅ Đã tạo TC (Expected sửa 2026-08-04, xem C-USR-04) | ❌ FAIL (R1+R2+R3, VR-007 2026-08-05): view-only đúng; 4 trường đúng (tên, avatar, phòng ban) nhưng thiếu MNV — vẫn tái hiện ở R3 (test cả 2 account Giang + TaiPM), gap thật xác nhận qua chat, xem C-USR-04 | 2026-08-05 |
| SC-USR-003 | Hiển thị phòng ban + khu vực trên hồ sơ | USR | DOC-v1.0-01 | P2 | UI | NEW | ✅ Đã tạo TC (Expected sửa 2026-08-04, xem C-USR-04) | ❌ FAIL (R1+R2+R3, VR-007 2026-08-05): có phòng ban đúng, thiếu MNV — vẫn tái hiện ở R3, gap thật (C-USR-04). Field "khu vực" đã bỏ khỏi Expected, xác nhận không cần | 2026-08-05 |
| SC-USR-004 | Hiển thị đúng 2 chỉ số (đơn giúp + quà nhận), không hiện điểm/tier/CO2 | USR | DOC-v1.0-01, DOC-v1.0-02 | P2 | Business Rule | NEW | ✅ Đã tạo TC | 🔶 MIXED (R1+R2+R3, VR-007 2026-08-05): 2 chỉ số PASS (case 0 re-confirm ở R3 qua account TaiPM); header 6 phần tử FAIL — thiếu MNV + badge tier "Hạng Đồng hành", **cả 2 xác nhận là gap thật qua chat (C-USR-04)**, không phải TC lỗi thời; số lớn (TC_02.7) vẫn BLOCKED thiếu data cả 3 round | 2026-08-05 |
| SC-USR-005 | Cấu hình kênh liên hệ sẽ lộ | USR | DOC-v1.0-01 | P3 | Functional | NEW | ✅ Đã tạo TC | ✅ PASS (R1+R2, VR-004): xác nhận KHÔNG có UI cấu hình kênh liên hệ, khớp C-USR-02 Resolved/Deferred | 2026-08-04 |
...
| SC-ORD-023 | Empty state khi danh sách rỗng (cả 2 tab) — text "Chưa có đơn nào" (⚠ UPDATE 2026-07-31, C-ORD-06 Resolved — có ảnh chụp thật) | ORD | Ảnh chụp thật app STG | P3 | UI | NEW | ✅ Đã update TC (2026-07-31) | — | — |
| SC-ORD-024 | Đơn "Đã huỷ" (CNL) hiển thị đúng tại tab "Đang diễn ra" (⚠ CORRECTED 2026-07-30) | ORD | Quan sát thực tế app | P2 | Business Rule | NEW | ✅ Đã tạo TC | — | — |
...
| SC-ORD-034 | Icon quay lại tại màn Chi tiết tin (⚠ NEW 2026-07-30) | ORD | Quan sát thực tế app STG | P3 | UI | NEW | ✅ Đã tạo TC | — | — |
| SC-ORD-035 | Icon quay lại/đóng tại màn "Đăng tin mới" (⚠ NEW 2026-07-30) | ORD | Quan sát thực tế app STG | P3 | UI | NEW | ✅ Đã tạo TC | — | — |
| SC-ORD-036 | Icon quay lại/đóng tại Wizard đăng tin — Bước 1/3 (⚠ NEW 2026-07-30) | ORD | Quan sát thực tế app STG | P3 | UI | NEW | ✅ Đã tạo TC | — | — |
| SC-ORD-037 | Empty state section "Đơn của tôi" (Trang chủ) — CORRECTED, section vẫn hiện + text "Chưa có đơn nào" (⚠ NEW 2026-07-31) | ORD | Ảnh chụp thật app STG | P3 | UI | NEW | ✅ Đã sửa TC (`TC_05.15`, 2026-07-31) | — | — |
...
| SC-ASN-005 | Tin ẩn khỏi Bảng tin sau khi rời "Chờ ghép" (ghép/hết hạn/huỷ) | ASN | DOC-v1.0-01 | P1 | Business Rule | NEW | ✅ Đã tạo TC | — | — |
...
| SC-ASN-013 | Hệ thống chỉ bắn tối đa 5 thông báo khớp tin cho Carrier, tính riêng theo từng tin — không cộng dồn theo ngày (⚠ NEW 2026-07-29, BA xác nhận rõ, thay thế SC-NTF-006) | ASN | DOC-v1.0-01, BA xác nhận qua chat | P2 | Business Rule | NEW | ✅ Đã tạo TC (TC_03.1-3, sheet NTF) | 🚫 BLOCKED (R1, Thongbao_VR-006 — recheck R2 không đổi) — cần multi-role orchestration | 2026-08-04 |
...
| SC-ASN-015 | Empty state section "Tin mới" (Trang chủ) khi chưa có tin phù hợp (⚠ NEW 2026-07-31) | ASN | Ảnh chụp thật app STG | P3 | UI | NEW | ✅ Đã tạo TC (`TC_05.34`, 2026-07-31) | — | — |
...
| SC-DLV-015 | Icon quay lại tại màn Theo dõi đơn (⚠ NEW 2026-07-30) | DLV | Quan sát thực tế app STG | P3 | UI | NEW | ✅ Đã tạo TC | — | — |
...
| SC-GIFT-003 | Card "Quà đã nhận" chỉ load đúng loại đã nhận (không hiện loại chưa nhận) | GIFT | Quan sát thực tế app | P3 | Business Rule | NEW | ✅ Đã tạo TC | 🔶 MIXED (R1+R2+R3, VR-007 2026-08-05): TC_02.12 ✅ PASS (từ R2). **TC_02.13 nay ✅ PASS ở R3** — account Giang có đủ dữ liệu thật 4/4 loại quà (Bông hoa 1, Ly cà phê 1, Gấu bông 2, Vương miện 1), card hiển thị đúng không thiếu loại nào. TC_02.14 (số lớn ≥12) vẫn 🚫 BLOCKED cả 3 round — thiếu data dàn dựng (cao nhất hiện có = 2) | 2026-08-05 |
...
| SC-GIFT-005 | Menu "Quà đã nhận" tại Cá nhân điều hướng sang màn Quà đã nhận | GIFT | Quan sát thực tế app | P3 | Functional | NEW | ✅ Đã tạo TC | ✅ PASS (R1+R2, VR-004) | 2026-08-04 |
| SC-GIFT-006 | Màn "Quà đã nhận" rỗng khi chưa nhận quà nào — text "Hiện tại chưa có dữ liệu" + subtitle (⚠ UPDATE 2026-08-05: title đổi lại theo xác nhận user, KHÔNG còn "Chưa có quà nào") | GIFT | DOC-v1.0-05 (quan sát trực tiếp app STG) | P3 | UI | NEW | ✅ Đã update TC (2026-08-05) | ✅ PASS (R3 VR-007 2026-08-05): title thật = "Hiện tại chưa có dữ liệu" — **user xác nhận trực tiếp đây KHÔNG phải bug, chấp nhận text hiện tại là đúng**, đã sửa lại Expected Result `TC_02.11` khớp thực tế (không còn kỳ vọng "Chưa có quà nào" nữa). Icon + subtitle đúng. R1 (VR-004, 2026-08-04) từng ghi FAIL dưới Expected cũ — giữ nguyên lịch sử round đó, không rewrite, vì đúng theo spec tại thời điểm test. | 2026-08-05 |
| SC-GIFT-007 | Icon quay lại tại màn "Quà đã nhận" | GIFT | Quan sát thực tế app | P3 | UI | NEW | ✅ Đã tạo TC | ✅ PASS (R1+R2, VR-004) | 2026-08-04 |
| SC-GIFT-008 | Icon quay lại tại màn Tặng quà (⚠ NEW 2026-07-30) | GIFT | Quan sát thực tế app STG | P3 | UI | NEW | ✅ Đã tạo TC | — | — |
...
| SC-NTF-001 | Thông báo khi ghép ngay (NTF-01/02) + tap → điều hướng Theo dõi đơn (⚠ UPDATE 2026-07-31, 3 biến thể tách 3 TC) | NTF | DOC-v1.0-01 | P2 | Functional | NEW | ✅ Đã tạo TC (+3, 2026-07-31) | 🔶 PARTIAL (R1, Thongbao_VR-006 — recheck R2 không đổi): 1 PASS (TC_03.33) + 2 BLOCKED (TC_03.31/32, loại thông báo chưa có trong tài khoản test) | 2026-08-04 |
| SC-NTF-002 | Thông báo khi khớp tuyến OFFER (NTF-03) + tap → điều hướng Chi tiết tin (⚠ UPDATE 2026-07-31) | NTF | DOC-v1.0-01 | P2 | Functional | NEW | ✅ Đã tạo TC (+1, 2026-07-31) | 🚫 BLOCKED (R1, Thongbao_VR-006 — recheck R2 không đổi) — loại thông báo chưa có trong tài khoản test | 2026-08-04 |
| SC-NTF-003 | Thông báo theo mốc vận chuyển: lấy hàng/đã giao/hoàn tất (NTF-04/05/06) + tap → điều hướng Theo dõi đơn (⚠ UPDATE 2026-07-31, 3 mốc tách 3 TC) | NTF | DOC-v1.0-01 | P2 | Functional | NEW | ✅ Đã tạo TC (+3, 2026-07-31) | ✅ PASS (R2, VR-006): 3/3 PASS (TC_03.35/36 từ R1 + TC_03.37 chuyển PASS ở R2 — loại thông báo "Đơn đã hoàn tất" đã xuất hiện) | 2026-08-05 |
| SC-NTF-004 | Thông báo khi nhận quà cảm ơn (NTF-07) + tap → điều hướng màn "Quà đã nhận" (⚠ UPDATE 2026-07-31) | NTF | DOC-v1.0-01 | P3 | Functional | NEW | ✅ Đã tạo TC (+1, 2026-07-31) | 🚫 BLOCKED (R1, Thongbao_VR-006 — recheck R2 không đổi) — loại thông báo chưa có trong tài khoản test | 2026-08-04 |
| SC-NTF-005 | Thông báo khi đơn huỷ (NTF-08) và tin quá hạn (NTF-09) — nhánh huỷ + tap → điều hướng Theo dõi đơn (⚠ UPDATE 2026-07-31); nhánh tin quá hạn (NTF-09) tạm chưa xác nhận hành vi tap | NTF | DOC-v1.0-01 | P2 | Functional | NEW | ✅ Đã tạo TC (+1, 2026-07-31) | ✅ PASS (R2, VR-006): TC_03.39 chuyển PASS — loại thông báo "Đơn đã bị huỷ" đã xuất hiện, tap đúng "Theo dõi đơn" | 2026-08-05 |
...
| SC-NTF-007 | Empty state khi chưa có thông báo nào — text "Chưa có thông báo nào" (⚠ UPDATE 2026-07-31, mở rộng C-ORD-06, Resolved — có ảnh chụp thật) | NTF | Ảnh chụp thật app STG | P3 | UI | NEW | ✅ Đã update TC (2026-07-31) | 🚫 BLOCKED (R1, Thongbao_VR-006 — recheck R2 không đổi) — tài khoản test có sẵn dữ liệu, không đưa về rỗng được qua UI | 2026-08-04 |
| SC-NTF-008 | Đánh dấu đã đọc (tap 1 thông báo / nút mark-all, C-NTF-03 Resolved) | NTF | DOC-v1.0-04 / Quan sát thực tế app | P3 | Functional | NEW | ✅ Đã tạo TC | ✅ PASS (R1, Thongbao_VR-006 — recheck R2 không đổi): TC_03.20-23 đều PASS | 2026-08-04 |
| SC-NTF-009 | Scroll xuống load thêm dữ liệu (phân trang, C-NTF-03 Resolved) | NTF | Quan sát thực tế app | P3 | Functional | NEW | ✅ Đã tạo TC | 🔶 PARTIAL (R2, VR-006): 4 PASS (TC_03.24 R1 + TC_03.25/26/27 chuyển PASS ở R2 — data đủ sâu để quan sát cơ chế load-more) + 1 BLOCKED (TC_03.28, không canh đúng timing "đang load" — list đã hết dữ liệu trước khi cắt mạng) | 2026-08-05 |
| SC-NTF-010 | Nội dung push không chứa SĐT (kiểm soát lộ liên hệ, OPR-07, ⚠ NEW 2026-07-30) | NTF | DOC-v1.0-01 | P1 | Business Rule | NEW | ✅ Đã tạo TC | 🚫 BLOCKED (R1, Thongbao_VR-006 — recheck R2 không đổi) — loại thông báo SĐT-lộ chưa có trong tài khoản test | 2026-08-04 |
| SC-NTF-011 | Icon quay lại tại màn Thông báo (⚠ NEW 2026-07-30) | NTF | Quan sát thực tế app STG | P3 | UI | NEW | ✅ Đã tạo TC | ✅ PASS (R1, Thongbao_VR-006 — recheck R2 không đổi): TC_03.18 | 2026-08-04 |
...
| 19 | C-ORD-06 | Ảnh chụp thật app STG (2026-07-31); riêng nhánh GIFT ghi đè bởi quan sát trực tiếp app STG qua vibe-test (2026-08-05, xem cập nhật cuối) | Empty state của màn Hoạt động (Đơn của tôi), màn Quà đã nhận, và màn Thông báo khi không có data — chưa có trong BRD/PRD/Figma, không có text chính thức. **Cập nhật 2026-07-29:** rà lại toàn bộ 82 ảnh Figma (DOC-v1.0-04) + BRD + demo docx — KHÔNG tìm thấy màn empty-state nào trong bất kỳ nguồn nào; nhãn DOC Source "Quan sát thực tế app" trước đây (2026-07-28) không có bằng chứng ảnh/tài liệu cụ thể đi kèm nên bị revert lại. **Cập nhật 2026-07-31:** user bổ sung 4 ảnh chụp THẬT app STG (status bar/pin/wifi thật, không phải mockup) vào `00_input/v1.0/Ảnh danh sách rỗng app stag thật/` — hoá ra 3 màn dùng **3 text RIÊNG BIỆT** (không phải 1 text chung như bản placeholder trước): Hoạt động = "Chưa có đơn nào", Thông báo = "Chưa có thông báo nào", Quà đã nhận = "Chưa có quà nào" + subtitle "Quà bạn nhận được từ đồng nghiệp sẽ hiện ở đây" | User xác nhận trực tiếp qua ảnh chụp thật (2026-07-31) — lần này có bằng chứng file cụ thể đi kèm, khác lần Resolved 2026-07-28 trước đó (chỉ mô tả qua chat, không kèm ảnh, đã bị revert) | **Resolved** cho nhánh Hoạt động (`SC-ORD-023`) + Thông báo (`SC-NTF-007`) — có bằng chứng ảnh chụp thật app STG. **Nhánh GIFT (`SC-GIFT-006`, màn "Quà đã nhận") đã ĐIỀU CHỈNH LẠI 2026-08-05:** vibe-test Round 3 (Profile_VR-007) quan sát trực tiếp app STG live (không phải ảnh chụp cũ) cho thấy title thật = "Hiện tại chưa có dữ liệu" (KHÔNG phải "Chưa có quà nào" như ảnh chụp 2026-07-31) — **user xác nhận trực tiếp đây KHÔNG phải bug, chấp nhận text hiện tại là đúng** (có thể ảnh chụp 2026-07-31 đã lỗi thời/khác build). Đã sửa lại `TC_02.11` khớp text mới. Vẫn Resolved, chỉ đổi nội dung text kỳ vọng cho riêng nhánh GIFT. | 2026-08-05 | REQ-ORD-011/SC-ORD-023, REQ-GIFT-001/SC-GIFT-006, REQ-NTF-001/SC-NTF-007 |
...
| 26 | C-USR-04 | Vibe-test VR-004 (app STG thật, 2026-08-04) | Header màn Cá nhân thiếu 2 phần tử so với Expected đã có sẵn từ trước (`TC_02.2`/`TC_02.3`): (1) trường MNV (mã nhân viên, định dạng "Phòng [ban] · MNV: [mã NV]"), (2) badge tier "🏆 Hạng Đồng hành" (display-only, theo tinh thần `C-USR-01` Resolved) | User xác nhận trực tiếp qua chat (2026-08-04) ngay sau khi nhận vibe-report: **cả 2 đều là gap thật, KHÔNG phải TC lỗi thời** — SĐT/khu vực/kênh liên hệ (3 field khác cũng bị FAIL cùng lúc ở `TC_02.1`) **KHÔNG phải gap**, đã sửa Expected `TC_02.1`/`TC_02.2` bỏ 3 field đó | **Open — đã push Jira 2026-08-04: [`FE-148`](https://foxproject.atlassian.net/browse/FE-148) (`BUG-003`, gộp cả MNV + badge tier vào 1 bug theo yêu cầu user), assignee Tuanvm37, chờ dev fix (verify đủ cả 2 phần trước khi đóng)** | 2026-08-04 | REQ-USR-002/003/004, SC-USR-002/003/004, TC_02.1/2/3 (giữ nguyên FAIL) |
...
| — (REVIEW ONLY, không phát sinh TC mới) | 2026-07-30 | 323 | `ISC_FoxEco_v1.0_TC_v1_R1.xlsx` + alias — `/review-tc` ROUND 2 full re-review (điểm chính thức, thay placeholder "chờ --recheck") | — | — | — | ✅ **85/100 (CONDITIONAL) — điểm CHÍNH THỨC.** Verify 12/12 finding Update trước = Fixed thật, 0 regression. Sweep R1 fresh 323 TC/9 sheet: 0 finding, 0 lỗi công thức, 9/9 Coverage Matrix khớp. **3 finding MỚI:** N1 (Major) — Dashboard cột AB/AC/AD + Summary báo SAI 100% đã chạy (45/187/91=323/323) trong khi Executed thật = 0 (vibe-test NOT_STARTED) — **regression** của bug `COUNTIFS(...,"<>")` đã fix 1 lần ở `RTM!F` (2026-07-29, đổi sang `SUMPRODUCT`) nhưng không propagate sang Dashboard khi consolidate 9 module; N2 (Minor) — RTM thiếu hẳn row cho 5/46 Req ID (`REQ-ASN-006`/`REQ-GIFT-002`/`REQ-TS-001`/`REQ-TS-002`/`REQ-USR-001`, đều có lý do out-of-scope đã biết, không phải gap thật) trái với governing note của chính RTM ("0 TC = tô đỏ", không phải ẩn row); N3 (Minor) — `TC-MASTER-LATEST.xlsx` chưa nhận commit rename tab `3be4b01`, lệch khỏi 2 file chính (vẫn byte-identical với nhau), phá cam kết "3 file byte-identical". Score = 100-(0×5+1×3+2×1)=95 → cap Direct-mode 85 → **85/100**. Vẫn Direct mode (`review-agent/AGENT.md` vẫn không tồn tại). Report: `11_tc-review/review-report-v1.0.md` §ROUND 2 |
| — (UPDATE theo yêu cầu user qua chat) | 2026-07-30 | 324 | `fragments/TC-HUYDON-v1.0.xlsx` (Mã CN TC_08) — +1 TC `TC_08.24` "Check LỊCH SỬ ghi log khi Người nhận huỷ đơn" (mirror `TC_08.22`/`TC_08.23`, đóng nốt gap 3/3 vai trò cho phần LỊCH SỬ; gắn cảnh báo DỰ KIẾN FAIL theo `C-CANCEL-2` như 2 TC anh em); sửa `TC_08.1` Pre-condition + Step 1 (mở rộng "chỉ Carrier" → "Sender/Carrier/Receiver", user xác nhận cả 3 role cùng xem 1 popup hoàn chỉnh; Steps 2-3/Expected/Title giữ nguyên wording Carrier-specific theo đúng yêu cầu user). Coverage Matrix `SC-CNL-003` 7→8, footer 27→28. Áp đồng bộ lên `TC-MASTER-v1.0.xlsx`/`ISC_FoxEco_v1.0_TC_v1_R1.xlsx`/`TC-MASTER-LATEST.xlsx`. | High:12, Medium:11, Low:3 (Huỷ đơn) | comprehensive | B1, B2, B4, B6, B7 | ⏳ **Chưa qua review** — điểm 85/100 ROUND 2 tính trên baseline 323 TC (trước thay đổi này); cần `/review-tc --recheck` cho bộ 324 TC. |
| DOC-v1.0-01 §D7 (OPR-07) | 2026-07-30 | 326 | `fragments/TC-NTF-v1.0.xlsx` (Mã CN TC_03) — +2 TC `TC_03.28`/`TC_03.29` "Check nội dung push không hiển thị SĐT" (mốc vừa MATCHED + mốc sau NTF-04), đóng gap RTM `REQ-NTF-002` (0→2 TC) sau khi user yêu cầu qua chat. Trước đó derive **scenario mới `SC-NTF-010`** (`test_scenario_map.md` + `MEMORY.md §4`, +1 requirement_traceability.md) vì OPR-07 chưa có scenario đại diện. Coverage Matrix `Thông báo`: scenarios 9→10, TCs 27→29. Chưa có bằng chứng UI cho nội dung push thật (không có ảnh Figma) — cả 2 TC gắn cảnh báo cần vibe-test xác nhận trước khi coi Expected là final. | High:2 (Thông báo) | comprehensive | B1 | ⏳ Chưa qua review — cần `/review-tc --recheck` cho bộ 326 TC. |
| Quan sát thực tế app STG (QA GiangDC2, chat 2026-07-30) | 2026-07-30 | 327 | `fragments/TC-HOATDONG-v1.0.xlsx` (Mã CN TC_01) — **SỬA** `TC_01.4` (bổ sung "Đã huỷ" vào bộ dữ liệu trộn tab "Đang diễn ra"); **SỬA** `TC_01.20`→**`TC_01.21`** sau khi chèn thêm 1 TC (viết lại từ "Đã huỷ KHÔNG hiển thị" — SAI so với app thật — sang "Đã huỷ hiển thị ĐÚNG dạng card tại tab Đang diễn ra"); **+1 TC MỚI `TC_01.17`** "Check bấm vào card trạng thái Đã huỷ mở đúng Chi tiết tin" (chèn đúng vị trí nhóm click-behavior, 4 TC sau dịch +1 vị trí: cũ 17→18, 18→19, 19→20, 20→21). Root cause: `SC-ORD-024` trước đó ghi sai nhận định "đơn Đã huỷ biến mất khỏi Hoạt động" — user xác nhận trực tiếp trên app STG: đơn Đã huỷ VẪN hiện ở tab "Đang diễn ra" và CÓ bấm được (khác card "Hết hạn"). Coverage Matrix `Hoạt động`: `SC-ORD-021` 4→5 TC (thêm biến thể Đã huỷ), `SC-ORD-024` title sửa, footer 20→21. TC-MASTER tổng 326→**327** (Hoạt động 20→21). | Medium:2, Low:1 (Hoạt động) | comprehensive | B1 | ⏳ Chưa qua review — cần `/review-tc --recheck` cho bộ 327 TC. |
| Quan sát thực tế app STG (QA GiangDC2, chat 2026-07-30) | 2026-07-30 | 328 | `fragments/TC-TRANGCHU-v1.0.xlsx` (Mã CN TC_05) — **+1 TC MỚI `TC_05.21`** "Check card Đơn của tôi hiển thị badge Đã huỷ khi đơn vừa bị huỷ" (chèn đúng vị trí nhóm badge, sau `TC_05.20` Hoàn thành, 8 TC sau dịch ID +1: cũ 21→22...28→33). Đóng gap phát sinh từ finding module Hoạt động (2026-07-30): đơn "Đã huỷ" không ẩn card, badge Block Definition "Đơn của tôi" bổ sung 6→**6 giá trị** (thêm Đã huỷ). **SỬA `TC_05.15`** Precondition: làm rõ đơn Đã huỷ KHÔNG tính là "không có đơn active" (section vẫn hiện, không ẩn). Coverage Matrix `Trang chủ`: `SC-ORD-003` 11→12 TC, footer 32→33. TC-MASTER tổng 327→**328** (Trang chủ 32→33). | Medium:1 (Trang chủ) | comprehensive | B1 | ⏳ Chưa qua review — cần `/review-tc --recheck` cho bộ 328 TC. |
| Quan sát thực tế app STG (QA GiangDC2, chat 2026-07-30) | 2026-07-30 | 329 | `fragments/TC-DANGTIN-v1.0.xlsx` (Mã CN TC_04) — **+1 TC MỚI `TC_04.110`** "Check khoá chỉnh sửa khi tin đã Đã huỷ (CANCELLED)" — đóng gap phát hiện khi quét lại toàn bộ TC-MASTER (2026-07-30): enumeration khoá edit `SC-ORD-009` (`TC_04.106-109`) trước đó chỉ có 4/5 trạng thái (MATCHED/IN_TRANSIT/COMPLETED/EXPIRED), thiếu CANCELLED. Giả định logic (chưa có bằng chứng UI trực tiếp, cần vibe-test xác nhận). Coverage Matrix `Đăng tin`: `SC-ORD-009` 4→5 TC, footer 109→110. TC-MASTER tổng 328→**329** (Đăng tin 109→110). | Low:1 (Đăng tin) | comprehensive | B4 | ⏳ Chưa qua review — cần `/review-tc --recheck` cho bộ 329 TC. |
| Quan sát thực tế app STG (QA GiangDC2, chat 2026-07-30) | 2026-07-30 | 331 | `fragments/TC-BANGTIN-v1.0.xlsx` (Mã CN TC_06) — **+2 TC MỚI `TC_06.8`/`TC_06.9`** "Check tin biến mất khỏi Bảng tin ngay sau khi tự động chuyển Hết hạn" / "...bị Sender huỷ" — chèn đúng vị trí sau `TC_06.7`, trước `TC_06.10` (cũ `TC_06.8`), 25 TC sau dịch ID +2. Đóng gap phát hiện khi quét lại toàn bộ TC-MASTER: `TC_06.6` (cũ) chỉ cover 1/3 lý do tin rời "Chờ ghép" (Đã ghép) — thiếu Hết hạn + Đã huỷ, dù Bảng tin là feed CÔNG KHAI nên cả 3 lý do đều phải làm tin biến mất. Mở rộng `SC-ASN-005` cho khớp phạm vi mới. Coverage Matrix `Bảng tin`: `SC-ASN-005` 3→5 TC, footer 31→33. TC-MASTER tổng 329→**331** (Bảng tin & Chi tiết tin 31→33). | High:2 (Bảng tin) | comprehensive | B4 | ⏳ Chưa qua review — cần `/review-tc --recheck` cho bộ 331 TC. |
| Quan sát thực tế app STG (QA GiangDC2, chat 2026-07-30) | 2026-07-30 | 336 | `fragments/TC-BANGTIN-v1.0.xlsx`, `TC-THEODOIDON-v1.0.xlsx`, `TC-TANGQUA-v1.0.xlsx`, `TC-NTF-v1.0.xlsx`, `TC-DANGTIN-v1.0.xlsx` — **Audit icon Back toàn dự án theo yêu cầu user** ("quét lại tất cả sheet xem đã có TC icon Back của từng màn chưa"). Kết quả: chỉ 1/9 màn (Quà đã nhận, `SC-GIFT-007`/`TC_02.15`) có TC sẵn (có bằng chứng Figma); 6 màn khác từng bị bỏ qua theo `Project_rule.md §10.1` (thiếu bằng chứng). User xác nhận trực tiếp trên app STG: 5/6 màn có icon Back hoạt động đúng (Bảng tin — root tab — không có, loại khỏi danh sách). **+5 scenario mới** `SC-ORD-034` (Chi tiết tin), `SC-ORD-035` (Đăng tin mới), `SC-DLV-015` (Theo dõi đơn), `SC-GIFT-008` (Tặng quà), `SC-NTF-011` (Thông báo) + **5 TC tương ứng**: `TC_06.14`, `TC_04.4`, `TC_07.4`, `TC_09.3`, `TC_03.18`. Mỗi TC chỉ test 1 hành vi (bấm icon → điều hướng đúng về màn nguồn), không áp dụng technique B1-B8 (N/A, log rõ trong Coverage Matrix). TC-MASTER tổng 331→**336**. | Low:5 (rải 5 sheet) | comprehensive | N/A (display/navigation-only) | ⏳ Chưa qua review — cần `/review-tc --recheck` cho bộ 336 TC. |
| Quan sát thực tế app STG (QA GiangDC2, chat 2026-07-30) | 2026-07-30 | 337 | `fragments/TC-DANGTIN-v1.0.xlsx` (Mã CN TC_04) — **+1 TC `TC_04.5`** "Check icon quay lại/đóng tại Wizard đăng tin — Bước 1/3" (user xác nhận, khác nút "Quay lại" giữa các bước đã có) + scenario mới `SC-ORD-036`. **Tự phát hiện + khôi phục 1 bug do thao tác chèn TC trước gây ra:** row-rotate dùng sai `LAST_ROW` (chưa cập nhật sau lần chèn TC_04.4) khiến `TC_04.111` cũ (case khoá chỉnh sửa "Đã huỷ/CANCELLED") bị mất khi rotate — đã khôi phục nguyên văn thành `TC_04.112`, verify đủ lại 5/5 case (MATCHED/IN_TRANSIT/COMPLETED/EXPIRED/CANCELLED). Coverage Matrix Đăng tin: `SC-ORD-036` +1, footer 111→112. TC-MASTER tổng 336→**337**. | Low:1 (Đăng tin) | comprehensive | N/A (display/navigation-only) | ⏳ Chưa qua review — cần `/review-tc --recheck` cho bộ 337 TC. |
| — (REVIEW ONLY, không phát sinh TC mới) | 2026-07-30 | 337 | `ISC_FoxEco_v1.0_TC_v1_R1.xlsx` + alias — `/review-tc --recheck` (điểm chính thức ROUND 3) | — | — | — | ✅ **85/100 (CONDITIONAL) — không đổi so với ROUND 2.** Re-check 3 finding cũ: **0/3 Fixed** — N1 (Dashboard/Summary sai 100% đã chạy), N2 (RTM thiếu 5/46 Req ID), N3 (`TC-MASTER-LATEST.xlsx` lệch tên tab) đều **Still Open**, không ai chủ động sửa (phiên này tập trung bổ sung coverage, không đụng Dashboard/RTM/tab-rename). Ghi nhận thêm: 2 bug MỚI phát sinh trong phiên (mất 7 nhãn Block sheet Đăng tin + mất 1 TC do row-rotate sai) — cả 2 đã tự phát hiện + tự sửa xong TRƯỚC khi recheck này chạy, không tính vào score. Sweep R1 fresh 337 TC/9 sheet × 3 file: 0 finding, 0 lỗi công thức, 9/9 Coverage Matrix khớp. Score = 100-(0×5+1×3+2×1)=95 → cap Direct-mode 85 → **85/100**, không đổi. Report: `11_tc-review/review-report-v1.0.md` §ROUND 3 |
...
| — (review-tc round 2/3 + coverage gap "Đã huỷ"/icon-Back + Dashboard fix, chưa log chi tiết ở bảng này) | 2026-07-30 | 337 | `ISC_FoxEco_v1.0_TC_v1_R1.xlsx` + 2 alias | 323→337 (+14, xem git log commit "Review-tc round 2/3, close Đã huỷ/icon-Back coverage gaps, fix Dashboard bug" — chi tiết per-TC không được ghi vào bảng này lúc đó, ghi nhận muộn tại đây để không đứt mạch trước khi thêm dòng dưới) | — | — | — |
| Ảnh chụp thật app STG + user xác nhận chat (2026-07-31) | 2026-07-31 | 346 | `fragments/TC-HOATDONG-v1.0.xlsx` (sửa `TC_01.19`/`TC_01.20` — text empty state), `fragments/TC-CANHAN-v1.0.xlsx` (sửa `TC_02.11` — text empty state), `fragments/TC-NTF-v1.0.xlsx` (sửa `TC_03.19` — text empty state; **+9 TC mới** `TC_03.31..39` — click thông báo → điều hướng, mở rộng `SC-NTF-001..005`). Consolidate lại toàn bộ → `ISC_FoxEco_v1.0_TC_v1_R1.xlsx` + 2 alias. **KHÔNG đụng** `fragments/TC-TRANGCHU-v1.0.xlsx` (chờ user trả lời câu hỏi khác về block "Đơn của tôi"/"Tin mới"). | +9 TC (Medium:8, Low:1) — 3 TC sửa text không đổi priority/count | standard | N/A | ⏳ Chưa qua review — cần `/review-tc --recheck` cho bộ 346 TC. |
| Ảnh chụp thật app STG (Trang chủ) + user xác nhận chat (2026-07-31) | 2026-07-31 | 347 | `fragments/TC-TRANGCHU-v1.0.xlsx` — **sửa `TC_05.15`** (CORRECTED, test sai rule "ẩn section" → sửa đúng "section vẫn hiện + text 'Chưa có đơn nào'", scenario `SC-ORD-037`); **+1 TC mới `TC_05.34`** (empty state "Tin mới", scenario `SC-ASN-015`). Coverage Matrix Trang chủ: 4→6 scenario, footer 33→34 (chỉ +1 TC thật, SC-ORD-037 dùng chung TC với SC-ORD-003 không cộng trùng). Consolidate lại → `ISC_FoxEco_v1.0_TC_v1_R1.xlsx` + 2 alias. TC-MASTER 346→**347**. | Low:1 (Trang chủ) | standard | N/A | ⏳ Chưa qua review — cần `/review-tc --recheck` cho bộ 347 TC. |
| — (data hygiene, không đổi scope/scenario) | 2026-07-31 | 347 | Toàn bộ 9 sheet TC-MASTER + fragment tương ứng — dọn 16 TC bỏ ghi chú/citation trong Expected Result; chuẩn hoá 108 cell DOC Source về ID chuẩn (+2 DOC ID mới `DOC-v1.0-07`/`DOC-v1.0-08`, xem `MASTER-MEMORY.md §2`) | — | N/A | N/A | ⏳ Chưa qua review — cần `/review-tc --recheck` cho bộ 347 TC (không đổi TC count, chỉ đổi nội dung). |
